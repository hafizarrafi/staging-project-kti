...
class SaleOrder(models.Model):
    ...
    include_certificates = fields.Boolean(
        string='Lampirkan Sertifikat Barang',
        default=False,
        help='Jika dicentang, PDF sertifikat dari produk akan otomatis dilampirkan saat print.'
    )
    ...
    carrier_id = fields.Many2one(
        'res.partner',
        string='Ekspedisi Pengiriman',
        domain=[('contact_type', '=', 'logistics')],
        help='Pilih kontak ekspedisi pengiriman.'
    )

    total_weight = fields.Float(
        string='Total Berat (kg)',
        compute='_compute_shipping_vals',
        store=True,
        help='Total berat produk dalam pesanan.'
    )

    estimated_shipping_cost = fields.Monetary(
        string='Estimasi Ongkir',
        compute='_compute_shipping_vals',
        store=True,
        help='Estimasi biaya pengiriman (Total Berat * Harga per kg Ekspedisi).'
    )

    @api.depends('order_line.product_id', 'order_line.product_uom_qty', 'carrier_id', 'carrier_id.shipping_price_per_kg')
    def _compute_shipping_vals(self):
        for order in self:
            total_weight = sum(line.product_id.weight * line.product_uom_qty for line in order.order_line if line.product_id)
            order.total_weight = total_weight
            if order.carrier_id:
                order.estimated_shipping_cost = total_weight * order.carrier_id.shipping_price_per_kg
            else:
